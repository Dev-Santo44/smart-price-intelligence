...
export default function UserRegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!orgSnap.exists()) {
        setError(`Organization "${domain}" does not exist.`);
        setLoading(false);
        return;
      }
      ...
      // ✅ 3. Wait for Firebase token (auth timing fix)
      ...
      if (err.code === "permission-denied") {
        setError("Permission denied — please check Firestore rules or authentication setup.");
      } else {
